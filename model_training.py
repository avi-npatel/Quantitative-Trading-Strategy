...
def train_trading_model(csv_filename):
    ...
    # --- UPGRADED TARGET LOGIC ---
    # Look 5 minutes into the future
    df['Future_Close_5m'] = df['Close'].shift(-5)
    
    # Require a minimum profit margin of 0.03% to classify as a 'Buy' (1)
    # This filters out market noise and flatline chopping
    profit_threshold = 0.0003 
    df['Target'] = (df['Future_Close_5m'] > (df['Close'] * (1 + profit_threshold))).astype(int)
    
    df.dropna(inplace=True)
    
    # Include our new advanced features
    ...
    # Increased the number of trees (estimators) to handle the new features
    ...
    # Let's see how many actual trades it suggested
